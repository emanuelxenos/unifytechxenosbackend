...
func NewRouter(db *database.PostgresDB, cfg *config.Config, hub *ws.Hub) *chi.Mux {
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Put("/api/produtos/bulk-preco", produtoHandler.AtualizarPrecosLote)
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/api/relatorios/dre", relatorioHandler.RelatorioDRE)
			r.Get("/api/relatorios/inadimplencia", relatorioHandler.RelatorioInadimplencia)
			r.Get("/api/relatorios/curva-abc", relatorioHandler.RelatorioCurvaABC)
			r.Get("/api/relatorios/comissoes", relatorioHandler.RelatorioComissoes)
			...
			r.Get("/api/relatorios/auditoria-estoque", relatorioHandler.AuditoriaMovimentacao)
		})
